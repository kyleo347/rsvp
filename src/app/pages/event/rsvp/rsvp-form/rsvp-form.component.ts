...
export class RsvpFormComponent implements OnInit, OnDestroy {
  ...
  private _setFormRsvp() {
    if (!this.isEdit) {
      // If creating a new RSVP, create new RsvpModel with default data
      ...
      // if an existing RSVP create new RsvpModel from existing data
      this.formRsvp = new RsvpModel(
        this.rsvp.userId,
        this.rsvp.name,
        this.rsvp.eventId,
  ...
  changeAttendanceSetGuests() {
    // if attendance set to no set guests = 0
